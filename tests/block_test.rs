use bitcoin::{transactions::{tx_out::TxOut, outpoint::{Outpoint}, tx_in::TxIn, transaction::Transaction}, compact_size_uint::CompactSizeUint, blocks::{block::Block, block_header::BlockHeader}};
...
fn create_txin(previous_output:Outpoint,height:Option<Vec<u8>>) -> TxIn{
    ...
    TxIn::new(previous_output, script_bytes, height, signature_script, sequence)
}
...
fn create_transaction(txin_count:CompactSizeUint,tx_in:Vec<TxIn>,txout_count:CompactSizeUint,tx_out:Vec<TxOut>) -> Transaction{
...
fn create_block_header()->BlockHeader{
    BlockHeader { version: (0x30), previous_block_header_hash: ([0;32]), merkle_root_hash: ([0;32]), time: (0x01), n_bits: (0x10), nonce: (0x20) }
}
...
fn test_seteo_de_utxos_dentro_de_un_bloque_con_3_transacciones_funciona_correctamente(){
    ...
    create_txins(hashes,indexs,&mut tx_in);
    // creacion de la transaccion 
    let second_transaction : Transaction = create_transaction(txin_count, tx_in, txout_count, tx_out);
    //creacion del bloque 
    let mut txn : Vec<Transaction> = Vec::new();
